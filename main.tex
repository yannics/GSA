...
\subsection{WTC I}
\label{codewtc}
\subsubsection{\texttt{scoreSnippet.sh}}
\label{scsh}
...
do
    echo "%---
#(set! paper-alist
  (cons '(\"extr\" . (cons (* 10 cm) (* 2 cm))) paper-alist))
\paper {
  #(set-paper-size \"extr\")
  top-margin = 0\cm
  left-margin = 0\cm
...
\header{
  ...
  << {
    ...
    $lilypond $bwv.ly
    $convert -density 600 $bwv.pdf $bwv.png
    $convert $bwv.png -gravity West -chop 130x0 bwv$bwv.png
    rm $bwv*
    bwv=$((bwv + 1))
done < $file

# The input file is formated as follow (with one `score' by line) :
# \\time 4/4 r8 c' d' e' f'8. g'32 f' e'8 a' d'[ g']\\laissezVibrer
# \\key c \\minor \\time 4/4 r8 c''16 b' c''8 g'8 aes' c''16 b'    c''8 d'' g'8 c''16 b' c''8 d''
# ...
\end{lstlisting}

\subsubsection{\texttt{midiCorpus.sh}}
\label{midcor}
\begin{lstlisting}[language=bash,basicstyle=\footnotesize\ttfamily]
#!/bin/bash
file=$1
...
# The input file is formated as follow (with one `score' by line) :
# c'8 d' e' f'8. g'32 f' e'8 a' d' g'
# c''16 b' c''8 g' aes' c''16 b' c''8 d'' g' c''16 b' c''8 d''
# ...
#--------------------------------------
# (let* ((dir ".../corpus/")
#        (stdout (format nil "~a" (UIOP:run-program (format nil
#           "sh -c 'cd ~S; for f in *.midi; do echo $f; done'" dir)
# 	    :output :string)))       
#        (dirc (M2T::string-to-list stdout)))
#   (loop for file in dirc collect (M2T::mat-trans (M2T::midi2mds
# 	    (M2T::scoring-midi (concatenate 'string dir (format nil
#	    "~(~a~)" file))) :to 'N3))))
\end{lstlisting}
%#############################################################################
...
$H.e^{\displaystyle \color{red}-x\color{black} \, \left | ln ( L/H ) \right |}$  with $H=$ highest frequency and $L=$ lowest frequency.
...
To solve the last two points from a sound file, I have taken the liberty of applying a high-pass filter according to the `opposite' exponential progression, as seen previously with the low-pass filter:
\smallskip

$H.e^{\displaystyle \color{red}(x-1)\color{black} \, \left | ln ( L/H ) \right |}$
%%#############################################################################
