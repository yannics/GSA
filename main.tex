...
\end{lstlisting}
%#############################################################################
\subsection{\texttt{corpus.sh}}
\label{code:corpus}
\begin{lstlisting}[language=bash,basicstyle=\footnotesize\ttfamily]
#!/bin/bash
usage(){
    echo "Usage: corpus.sh {-midi|-score} <inFile> [<startNumber>]"
}
arg=$1
file=$2
# http://lilypond.org
...
# lilypond version 
version=`$lilypond -version | sed -n '1p' | awk '{print $NF}'`
# start number
...
do
    echo "%---
\version \"$version\"" > $bwv.ly
    if [ $arg = "-score" ]
    then	 
	echo "#(set! paper-alist
