...
A sine wave is generated according to the water drop \texttt{rad}ius in the millimeter unit $\Rightarrow$ \texttt{FSinOsc.ar(3/(rad/1000))}. The frequency is estimated as $f=3/r$ with $r$ the water drop radius in meters \citep{kvdd}. The volume envelope is defined by $\Rightarrow$ \texttt{Env.new([1, 0], [0.1])}, and the pitch envelope by $\Rightarrow$ \texttt{Env.new([0, 2], [atk])}

\smallskip

Note that \texttt{atk} can be interpreted as a surface type where the water drop falls and/or as a damp value.
...
% source : https://pixabay.com/photos/drops-of-water-water-liquid-fresh-578897/
% Canon EOS 7d -- 90.0mm · ƒ/25.0 · 1/250s · ISO 640 ↯
% Image type: JPG -- Resolution:	3309×2206 -- Created: April 8, 2012 -- Uploaded: Dec. 28, 2014
% by Rony Michaud  --  Montreal/Canada  -- Pixabay Member since Dec. 14, 2014
\label{dropwater}
\end{center}
\end{figure}
