...
%  \bigskip
%  
%  In this model, the proportion concerns as well as the duration of the sample than the position in time of the acme of the climax. 
% Therefore, the algorithm consists to compute the duration of each voice according to a given ratio, and their respective delay in order to match the acme of the climax.
% 
%  In this order, there are two ways to generate the proportional canon as an event climax:
%  
%  \begin{enumerate}
%\item according to the first voice $A$ and the last voice $\rho A$, then each intermediary voice will be a linear interpolation between the first voice with a delay of zero second and $d$.
%\item by applying the ratio $\rho$ for each voice such as:
%\end{enumerate}
%
%  
%  \bigskip
