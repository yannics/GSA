...
\end{lstlisting}
\item The conversion midi to MDS can be done as a raw data -- see \textsl{Outline of the Standard MIDI File Structure} in appendice \fullref{infomidi} -- using tools of the Common Lisp library \textsl{cl-gsa} which is described in the two next sections \textsl{Duration} and \textsl{Score}. Note that this library requires in this case the package MIDI. 
